...
How **iocextract** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `iocextract` lives in the **Threat intel / detections** cluster.
...
| from | relation | to |
|---|---|---|
| `iocextract` | JSON findings compose with | [`stixgen`](https://github.com/cognis-digital/stixgen), [`attackmap`](https://github.com/cognis-digital/attackmap), [`ttphunt`](https://github.com/cognis-digital/ttphunt) |
| `iocextract` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `iocextract` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `iocextract` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the threat intel / detections cluster.** `iocextract` output feeds the next tool:
```bash
iocextract ... --format json > out.json          # this tool's findings (see `iocextract --help`)
...
iocextract ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
iocextract ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
iocextract ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `iocextract`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `iocextract` slots
