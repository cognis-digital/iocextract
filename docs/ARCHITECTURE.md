...
- **rules/analyzers** apply the heuristics shipped in `iocextract/core.py`.
- **score** ranks by severity.
- **MCP server** (`iocextract mcp`) exposes `scan` for Cognis.Studio agents.
