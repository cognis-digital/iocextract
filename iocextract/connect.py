"""Native cognis-connect emit for iocextract — forward findings to any platform.

Maps iocextract's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    iocextract ... --format json | iocextract-emit --to stix
    iocextract-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a iocextract record toward the Finding contract before normalize().
